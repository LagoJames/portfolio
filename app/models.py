...
class HireRequest(db.Model):
    ...
    deadline_flexibility = db.Column(db.String(20), default='soft')

    # Payment details
    payment_method = db.Column(db.String(50))  # mpesa, stripe, wise
    ...
    scope_change_budget = db.Column(db.String(30), default='fixed')
    tip_amount = db.Column(db.Float)
    tip_percent = db.Column(db.String(10))  # '0','5','10','15','20','custom'
    no_rush = db.Column(db.Boolean, default=False)
    mpesa_confirmation = db.Column(db.Text)
    ...
    payment_status = db.Column(db.String(30), default='not_required')
    # Values: not_required, awaiting_confirmation, confirmed
    ...
    invoice_opt_out = db.Column(db.Boolean, default=False)
...
class HireRequestFile(db.Model):
    ...
    file_kind = db.Column(db.String(50), default='supporting')
...
class Invoice(db.Model):
    ...
    invoice_type = db.Column(db.String(20), nullable=False)  # 'invoice' or 'receipt'
    pdf_path = db.Column(db.String(500))
    sent_at = db.Column(db.DateTime)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    hire_request = db.relationship('HireRequest', backref='invoices')
...
class PMProject(db.Model):
    ...
    expenses = db.relationship('PMExpense', backref='project', lazy=True)
    ...
    @property
    def paid(self):
        return sum(p.amount for p in self.payments if p.amount) or 0.0

    @property
    def balance(self):
        if not self.budget:
            return 0.0
        return max(0.0, self.budget - self.paid)
...
class PMExpense(db.Model):
    ...
    category = db.Column(db.String(50))
    notes = db.Column(db.Text)
    project_id = db.Column(db.Integer, db.ForeignKey('pm_projects.id'), nullable=True)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))


class PMDailyFocus(db.Model):
    __tablename__ = 'pm_daily_focus'
    id = db.Column(db.Integer, primary_key=True)
    date = db.Column(db.Date, nullable=False)
    task_id = db.Column(db.Integer, db.ForeignKey('pm_tasks.id'), nullable=False)
    est_time = db.Column(db.String(50))          # e.g. "2 hrs", "30 min"
    status = db.Column(db.String(30), default='not_started')
    notes = db.Column(db.Text)                   # PM instructions for freelancer
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))

    task = db.relationship('PMTask', backref='daily_focus_entries', lazy=True)
