...
def create_app():
    ...
    if app.config.get('IS_PRODUCTION') and app.config.get('STRIPE_SECRET_KEY') and not app.config.get('STRIPE_WEBHOOK_SECRET'):
        raise RuntimeError('STRIPE_WEBHOOK_SECRET must be set in production when Stripe is enabled.')
    ...
    def inject_settings():
        """Make site settings available in every template as `settings` dict."""
        try:
    ...
    from app.routes.crm.samples import crm_samples
    ...
    app.register_blueprint(crm_samples, url_prefix='/admin/samples')
    ...
    def reset_admin(email, password):
        """Delete all admin accounts and create a fresh one."""
        import bcrypt
        from app.models import AdminUser
        AdminUser.query.delete()
        hashed = bcrypt.hashpw(password.encode(), bcrypt.gensalt()).decode()
        db.session.add(AdminUser(email=email, password_hash=hashed))
        db.session.commit()
        click.echo(f"Admin reset: {email}")

    # WhatsApp visitor tracking
    from app.utils.whatsapp import track_visitor, start_scheduler
